...
abstract class AbstractScanner implements Scanner {
    ...
    if (!inRecoveryOption) {
      ...
      switch (openKind) {
        case OPEN_SQUARE_BRACKET_TOKEN:
          ...
          break;
        case OPEN_DOUBLE_SQUARE_BRACKET_TOKEN:
          type = TokenType.CLOSE_DOUBLE_SQUARE_BRACKET;
          break;
  ...
  int bigSwitch(int next) {
    ...
    if (identical(next, $CLOSE_SQUARE_BRACKET)) {
      return tokenizeCloseSquareBracket(next);
    }
  ...
  int tokenizeOpenSquareBracket(int next) {
    ...
    if (identical(next, $OPEN_SQUARE_BRACKET)) {
      next = advance();
      appendBeginGroup(TokenType.OPEN_DOUBLE_SQUARE_BRACKET);
      return next;
    } else if (identical(next, $CLOSE_SQUARE_BRACKET)) {
  ...
  int tokenizeCloseSquareBracket(int next) {
    // ] ]]
    next = peek();
    if (identical(next, $CLOSE_SQUARE_BRACKET)) {
      advance();
      return appendEndGroup(TokenType.CLOSE_DOUBLE_SQUARE_BRACKET,
  ...
  int tokenizeColon(int next) {
    // : ::
    return select($COLON, TokenType.DOUBLE_COLON, TokenType.COLON);
  }
